...
# 貼文瀏覽的表單介面調整
    * 高度要再增加，以便容納所有貼文列加上表單最下方的換頁按鈕群    
    * 清單的標題欄要加寬，大約是目前寬度的兩倍
    * 表單寬度也要增加，因應標題欄的加寬

> 貼文瀏覽的 "依爬蟲任務" 選取時，沒有選項，是否是因為透過爬蟲任務爬取貼文時，並沒有記錄該貼文是由該爬蟲任務所爬取的？
    * 若是的話，則需要在貼文的資料上加上這個欄位
    * 或是你有其他的建議

# 貼文瀏覽加上 "分析 Line ID" 的功能
    * 整合 LineIDAnalyzer 的分析功能
        + 若需要設定 ChatGPT API Key，則在主表單增加 "設定" 功能以便設定
    * 分析時可選擇分析範圍
        + 目前這一頁裡的貼文
        + 所有貼文
    * 分析後顯示分析報告
        + 列出所有含有 Line ID 的貼文及分析出來的 Line ID
